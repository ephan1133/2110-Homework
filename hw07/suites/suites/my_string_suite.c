...
END_TEST
/******************************************************************************/
/**************************** remove_last_instance tests ********************************/
/******************************************************************************/
START_TEST(test_remove_last_instance_basic)
{
    char test_str[] = "abc";
    char char_to_remove = 'b';

    //remove 'b'
    remove_last_instance(test_str, char_to_remove);
...
START_TEST(test_remove_last_instance_random)
{
    char test_str[100];
    randStringGenerator(test_str, sizeof(test_str));
    ...
    // find the last instance of randChar
    int i = sizeof(test_str) - 1;
    while (i >= 0) {
        if (test_str[i] == randChar) {
            break;
        }
    ...
    remove_last_instance(test_str, randChar);

    // Checking if strncmp returns 0 for first half (before removed char)
    ck_assert_int_eq(strncmp(test_str, orig_str, i), 0);

    // Checking if strncmp returns 0 for second half (after removed char)
    ck_assert_int_eq(strncmp(test_str + i, orig_str + i + 1, sizeof(orig_str) - i - 1), 0);
}
END_TEST

START_TEST(test_remove_last_instance_instance_as_last_char)
{
    char test_str[100];
    randStringGenerator(test_str, sizeof(test_str));

    // Select the target char as the last char
    char remove_char = test_str[sizeof(test_str) - 2];
    
    char orig_str[100];
    strcpy(orig_str, test_str);
    
    remove_last_instance(orig_str, remove_char);

    // Checking if test string removed last char
    test_str[sizeof(test_str) - 2] = '\0';
    ck_assert_str_eq(test_str, orig_str);
}
END_TEST

START_TEST(test_remove_last_instance_one_char_string)
{
    char test_str[2];
    randStringGenerator(test_str, sizeof(test_str));
    ...
    remove_last_instance(test_str, remove_char);
...
START_TEST(test_remove_last_instance_no_instances)
{
    char test_str[100];
    randStringGenerator(test_str, sizeof(test_str));
...
Suite *my_string_suite(void)
{
    ...
    tcase_hack(s, setup, NULL, test_remove_last_instance_basic);
    tcase_hack(s, setup, NULL, test_remove_last_instance_random);
    tcase_hack(s, setup, NULL, test_remove_last_instance_instance_as_last_char);
    tcase_hack(s, setup, NULL, test_remove_last_instance_one_char_string);
    tcase_hack(s, setup, NULL, test_remove_last_instance_no_instances);
