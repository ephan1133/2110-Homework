...
#define INCORRECT_RESULT_MSG "array at index %d not removeulated correctly. Expected: %s, %d, %d (in order).\nargv[1]='%s'\n"

// Helper function prototypes
static void setup(void);
static void _enqueue(const char *studentName, const enum subject topicName, const float questionNumber, struct public_key pub_key);
...
/* enqueue() tests */
// Test that enqueue works when called once (this is the "basic" intro test, easy test case)
START_TEST(test_enqueue_single)
{
    ...
    // If we want to do anything particular for this test (maybe have some data already in the array, we should enqueue it here)
    // It's fine if we start from an empty array for this test, so nothing needed here either
    // Frequently, we have helper methods to help with setup. Check out the _enqueue function; this is
    //   entirely separate from the student's enqueue function and is essentially a "working" implementation
    //   (without error handling) that you can use to enqueue test data to the array.

    // Step 3: make the function call we're testing (in this case, enqueueUser)
    ...
    int retVal = enqueue(student_name, subject, questionNumber, pub_key);
...
START_TEST(test_enqueue_two)
{
    ...
    pub_key1.n = 1;
    pub_key1.e = 2;
    int retVal1 = enqueue(student_name1, subject1, questionNumber1, pub_key1);
    ...
    pub_key2.n = 7;
    pub_key2.e = 3;
    int retVal2 = enqueue(student_name2, subject2, questionNumber2, pub_key2);
    ...
    ck_assert_msg(oh_queue.students[1].studentData.topic.topicName == subject2, "Expected topicName to be %d, was %d", subject2, oh_queue.students[1].studentData.topic.topicName);
    ck_assert_msg(strncmp(oh_queue.students[1].studentData.name, student_name2, MAX_NAME_LENGTH) == 0, "Expected name to be %s, was %s", student_name2, oh_queue.students[1].studentData.name);
    ...
    ck_assert_msg(oh_queue.students[1].studentData.topic.questionNumber == questionNumber2, "Expected questionNumber to be %f, was %f", questionNumber2, oh_queue.students[1].studentData.topic.questionNumber);
    ck_assert_msg(oh_queue.students[1].queue_number == 1, "Expected queue_number to be 1 was %d", oh_queue.students[1].queue_number);
}
END_TEST
...
START_TEST(test_enqueue_multiple)
{
    ...
    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == 25, "Expected no_of_people_in_queue to be 25 after enqueueing 25 users, was %d", oh_queue.stats.no_of_people_in_queue);
...
START_TEST(test_enqueue_long_name)
{
...
START_TEST(test_enqueue_too_many_students)
{
    for (int i = 0; i < MAX_QUEUE_LENGTH; i++) {
        ...
        _enqueue(student_name, subject, questionNumber, pub_key);
    }
    ...
    int retVal = enqueue("One Mohr", LC3, 2.0, pub_key_temp);
    ck_assert_int_eq(retVal, FAILURE);

    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == MAX_QUEUE_LENGTH, "Expected no_of_people_in_queue to be %d after enqueueing %d users, was %d", MAX_QUEUE_LENGTH, MAX_QUEUE_LENGTH, oh_queue.stats.no_of_people_in_queue);
...
START_TEST(test_remove_basic)
{
    ...
    ck_assert_msg(oh_queue.stats.no_of_people_visited == 1, "Expected no_of_people_visited to be 1 after enqueueing 5 users and removing 1, was %d", oh_queue.stats.no_of_people_visited);
    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == 4, "Expected no_of_people_in_queue to be 4 after enqueueing 5 users and removing 1, was %d", oh_queue.stats.no_of_people_in_queue);
    for (int i = 1; i < 5; i++) {
        char student_name[10];
        sprintf(student_name, "sai%d", i);
        enum subject subject = Assembly;
...
START_TEST(test_remove_single_student)
{
    ...
    ck_assert_msg(oh_queue.stats.no_of_people_visited == 1, "Expected no_of_people_visited to be 1 after enqueueing 1 user and removing 1, was %d", oh_queue.stats.no_of_people_visited);
    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == 0, "Expected no_of_people_in_queue to be 0 after enqueueing 1 user and removing 1, was %d", oh_queue.stats.no_of_people_in_queue);
}
END_TEST

START_TEST(test_remove_no_students)
{
    int retVal = dequeue();
...
START_TEST(test_remove_multiple)
{
    for (int i = 0; i < 10; i++) {
    ...
    ck_assert_msg(oh_queue.stats.no_of_people_visited == 4, "Expected no_of_people_visited to be 4 after enqueueing 10 users and removing 4, was %d", oh_queue.stats.no_of_people_visited);
    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == 6, "Expected no_of_people_in_queue to be 6 after enqueueing 10 users and removing 4, was %d", oh_queue.stats.no_of_people_in_queue);
    for (int i = 4; i < 10; i++) {
        char student_name[20];
        sprintf(student_name, "2110 student%d", i);
        enum subject subject = Assembly;
...
START_TEST(test_remove_everything)
{
    for (int i = 0; i < 10; i++) {
        char student_name[20];
        sprintf(student_name, "2110 student%d", i);
    ...
    ck_assert_msg(oh_queue.stats.no_of_people_visited == 10, "Expected no_of_people_visited to be 10 after enqueueing 10 users and removeping 10, was %d", oh_queue.stats.no_of_people_visited);
    ck_assert_msg(oh_queue.stats.no_of_people_in_queue == 0, "Expected no_of_people_in_queue to be 0 after enqueueing 10 users and removeping 10, was %d", oh_queue.stats.no_of_people_in_queue);
}
END_TEST
...
START_TEST(test_group_by_topic_basic)
{
    ...
    pub_key1.n = 5;
    pub_key1.e = 6;
    _enqueue(student_name1, subject1, questionNumber1, pub_key1);
    ...
    pub_key2.n = 5;
    pub_key2.e = 7;
    _enqueue(student_name2, subject2, questionNumber2, pub_key2);
...
START_TEST(test_group_by_topic_empty_array) {
    ...
    // The enqueueed entry is no longer in the array
...
START_TEST(test_update_student_basic) {
    ...
    for (int i = 0; i < 2; i ++) {
        _enqueue(studentNames[i], topicNames[i], questionNumbers[i], pub_keys[i]);
    }
...
START_TEST(test_office_hours_status_students_enqueueed) {
    for (int i = 0; i < 5; i++) {
...
START_TEST(test_office_hours_status_all_students_removeped) {
    for (int i = 0; i < 8; i++) {
        char student_name[30];
        sprintf(student_name, "queue_student_%d", i);
        enum subject subject = Assembly;
...
START_TEST(test_office_hours_status_some_students_removeped) {
    for (int i = 0; i < 8; i++) {
        char student_name[30];
        sprintf(student_name, "queue_student_%d", i);
        enum subject subject = Assembly;
...
START_TEST(test_office_hours_status_remove_all_students_by_name) {
    ...
    _enqueue(studentNames[0], topicNames[0], questionNumbers[0], pub_keys[0]);
...
Suite *oh_queue_suite(void)
{
    ...
    // enqueueES
    tcase_hack(s, setup, NULL, test_enqueue_single);
    tcase_hack(s, setup, NULL, test_enqueue_two);
    tcase_hack(s, setup, NULL, test_enqueue_multiple);
    tcase_hack(s, setup, NULL, test_enqueue_null_terminator);
    tcase_hack(s, setup, NULL, test_enqueue_long_name);
    tcase_hack(s, setup, NULL, test_enqueue_too_many_students);

    // removeS
    tcase_hack(s, setup, NULL, test_remove_basic);
    tcase_hack(s, setup, NULL, test_remove_single_student);
    tcase_hack(s, setup, NULL, test_remove_no_students);
    tcase_hack(s, setup, NULL, test_remove_multiple);
    tcase_hack(s, setup, NULL, test_remove_everything);
    ...
    // QUEUE STATUS
    tcase_hack(s, setup, NULL, test_office_hours_status_students_enqueueed);
    tcase_hack(s, setup, NULL, test_office_hours_status_all_students_removeped);
    tcase_hack(s, setup, NULL, test_office_hours_status_some_students_removeped);
